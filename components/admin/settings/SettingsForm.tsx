...
export function SettingsForm({ settings }: { settings: StoreSettings }) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <StickyBar state={state} pending={isPending} />
    </form>
  )
}

/** Barra inferior fija con el botón de guardar y feedback de estado. */
function StickyBar({
  state,
  pending,
}: {
...
function FlashMessage({
  ...
  // Solo mostramos cuando hay un mensaje y no estamos enviando.
