...
export async function sendOrderConfirmationEmails(orderId: string) {
  ...
  const trackingUrl = `${PUBLIC_URL()}/seguimiento/${order.orderNumber}`
  const adminUrl = `${PUBLIC_URL()}/admin/pedidos/${order.id}`
...
export async function sendStatusUpdateEmail(
  ...
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    select: {
      orderNumber: true, firstName: true, lastName: true, email: true, trackingNumber: true,
    },
  })
  if (!order) return { ok: false, error: "Pedido no encontrado" }
